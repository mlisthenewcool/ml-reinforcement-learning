...
def rave_search(root_state, iter_max, heuristic=None, verbose=False):
    """
    ...
    for i in range(iter_max):  # start MC-RAVE
        node = rootnode
        ...
        while node.untried_moves == [] and node.childNodes != []:
            node = node.uct_select_child()  # todo, uct_select_child -> mc_rave_select_child()
